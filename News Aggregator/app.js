...
async function fetchNewsQuery(query) {
    if (!query) {
        console.error("Query is empty, cannot fetch news.");
        return [];
    }
    try {
...
function displayBlogs(articles) {
    ...
    if (articles.length === 0) {
        blogContainer.innerHTML = `<p>No articles found. Try a different search or category.</p>`;
    }
...
const imageUrl = article.urlToImage || "https://via.placeholder.com/600x400?text=No+Image"; // Use placeholder as fallback
img.src = imageUrl;
...
(async () => {
    try {
        const articles = await fetchCategoryNews("technology");
        displayBlogs(articles);
    } catch (error) {
